...
import warnings
from summarization_utils.residual_stream import ResidualStreamDataset
from summarization_utils.store import save_array, update_csv
...
def split_by_label(
    ...
    first_cluster = [adj[1:] for i, adj in enumerate(adjectives) if labels[i] == 0]
    second_cluster = [adj[1:] for i, adj in enumerate(adjectives) if labels[i] == 1]
...
def _fit(
    ...
    train_pos: str,
    train_layer: int,
    test_data: ResidualStreamDataset,
    test_pos: str,
    test_layer: int,
    n_init: int = 10,
    n_clusters: int = 2,
...
def train_classifying_direction(
    ...
    **kwargs,
    ...
    save_array(
        train_line,
        f"{method.value}_{train_data.prompt_type.value}_{train_pos}_layer{train_layer}",
        model,
    )
