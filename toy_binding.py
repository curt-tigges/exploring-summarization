...
PROMPT_TEMPLATE = (
    "{NAME_L} likes {OBJECT_L}. {NAME_R} likes {OBJECT_R}. The {OBJECT_Q} belongs to"
)
NAMES = [
    "Anne",
    "Bob",
...
def get_counterfactual_tuples(
    ...
    random.seed(seed)
    cf_tuples = []
    for idx, (name_l, object_l, name_r, object_r, object_q) in enumerate(prompt_tuples):
        if idx % 2 == 0:
            # patch object_l
            new_object_l = random.choice(
                [object for object in OBJECTS if object not in [object_l, object_r]]
            )
...
PROMPT_TUPLES = [
    ...
    for name_l, name_r in itertools.combinations(NAMES, 2)
    for object_l, object_r in itertools.combinations(OBJECTS, 2)
...
CF_PROMPTS = [
    ...
    for name_l, object_l, name_r, object_r, object_q in CF_TUPLES
]
ANSWERS = [
