...
class TestCircuitAnalysis(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.text = [
            f"The movie was {adj}. I thought it was"
            for adj in cls.pos_adjectives + cls.neg_adjectives
        ]
        cls.answers = [[" great", " terrible"]] * len(cls.pos_adjectives) + [
            [" terrible", " great"]
        ] * len(cls.neg_adjectives)
        cls.answer_tokens = cls.model.to_tokens(cls.answers, prepend_bos=False)
        cls.clean_tokens = cls.model.to_tokens(cls.text)
        cls.answer_tokens = einops.repeat(
            cls.answer_tokens, "batch correct -> batch 1 correct"
        )
    ...
    def test_adjective_tokens(self):
        for adj in self.pos_adjectives + self.neg_adjectives:
            ...
            self.assertEqual(
                len(adj_tokens), 2, f"Bad length {len(adj_tokens)} for {adj_tokens}"
            )
    ...
    def test_text_token_length(self):
        ...
        for s in self.text:
            self.assertEqual(
                len(self.model.to_str_tokens(s)),
                seq_len,
                (
    ...
    def test_logit_diff_calculation(self):
        clean_logit_diff = get_logit_diff(
            self.clean_logits, answer_tokens=self.answer_tokens
        )
        average_logit_diff = cache_to_logit_diff(
            self.clean_cache, self.answer_tokens, self.model, -1
        )
        self.assertLessEqual(
            torch.max(torch.abs(average_logit_diff - clean_logit_diff)), 1e-4
        )
    ...
    def test_accumulated_resid(self):
        ...
        self.assertEqual(
            clean_logit_lens_logit_diffs.shape, (self.model.cfg.n_layers + 1,)
        )
    ...
    def test_create_cache_for_dir_patching(self):
        ...
        self.assertTrue(
            torch.allclose(result_cache["result"], torch.tensor([1.0, 1.5]), atol=1e-5)
        )
        self.assertTrue(
            torch.allclose(
                result_cache["resid_pre"], torch.tensor([2.0, 2.5]), atol=1e-5
            )
        )
        self.assertTrue(
            torch.allclose(result_cache["other"], torch.tensor([3.5, 4.5]), atol=1e-5)
        )
...
class TestGetProbDiff(unittest.TestCase):
    def test_basic_functionality(self):
        ...
        expected = torch.tensor(
            [-0.19928]
        )  # Pre-computed value based on the given logits and tokens
    ...
    def test_per_prompt_true(self):
        ...
        expected = torch.tensor(
            [-0.19928]
        )  # Same as above because there's only one batch
        self.assertTrue(torch.allclose(result, expected, atol=1e-4))
...
class TestFinalNonPadToken(unittest.TestCase):
    ...
    def test_simple_case(self):
        ...
        expected = torch.tensor([[0.4, 0.5, 0.6], [0.3, 0.2, 0.1]])
        self.assertTrue(
            torch.allclose(
    ...
    def test_all_ones(self):
        ...
        expected = torch.tensor([[0.7, 0.8, 0.9], [0.3, 0.2, 0.1]])
        self.assertTrue(
            torch.allclose(
                get_final_non_pad_token(self.logits, attention_mask), expected
            )
    ...
    def test_single_batch(self):
        ...
        logits = torch.tensor([[[0.1, 0.2], [0.4, 0.5], [0.7, 0.8]]])
        attention_mask = torch.tensor([[1, 0, 1]])
        expected = torch.tensor([[0.7, 0.8]])
        self.assertTrue(
            torch.allclose(get_final_non_pad_token(logits, attention_mask), expected)
        )
